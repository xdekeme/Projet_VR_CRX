...
public:
	...
	Shader(const char* vertexPath, const char* fragmentPath, const char* geometryPath = nullptr)
	{
        ...
        if (geometryPath != nullptr) {
            ...
            try {
                ...
                std::cout << "ERROR::SHADER::GEOMETRY_FILE_NOT_SUCCESFULLY_READ: " << e.what() << std::endl;
            }
    ...
    Shader(const char* computeshaderPath)
	{
        ...
        try {
            // open file
            gShaderFile.open(computeshaderPath);
            std::stringstream gShaderStream;
            // read file's buffer contents into streams
            gShaderStream << gShaderFile.rdbuf();
            // close file handlers
            gShaderFile.close();
            // convert stream into string
            computeshaderCode = gShaderStream.str();
        } catch (std::ifstream::failure& e) {
            std::cout << "ERROR::SHADER::COMPUTESHADER_FILE_NOT_SUCCESFULLY_READ: " << e.what() << std::endl;
        }
        GLuint computeshader = compileShader(computeshaderCode, GL_COMPUTE_SHADER);
        ID = compileProgramComputeShader(computeshader);
	}
    ...
    void setMatrix4_count(const GLchar* name, const glm::mat4& matrices, size_t count) {
        int location = glGetUniformLocation(ID, name);
        glUniformMatrix4fv(location, static_cast<GLsizei>(count), GL_FALSE, glm::value_ptr(matrices));
    }
...
private:
    ...
    GLuint compileProgramComputeShader(GLuint computeshader)
    {
        GLuint programID = glCreateProgram();
        ...
        {
            glGetProgramInfoLog(programID, 1024, NULL, infoLog);
            std::cout << "ERROR::PROGRAM_LINKING_ERROR_COMPUTESHADER:  " << infoLog << std::endl;
        }
        return programID;
    }
