...
// #endif
	...
	const std::string sourceV = "#version 330 core\n"
		...
		"FragPosLightSpace = lightSpaceMatrix * frag_coord; \n"
		"\n" 
		"}\n";
	...
	const std::string sourceF = "#version 400 core\n"
		...
		"float lighting = light.ambient_strength + (1.0 - shadow)*attenuation*(diffuse + specular); \n"
		"FragColor = vec4(materialColour * vec3(lighting), 1.0); \n"
...
const std::string sourceFParticle = "#version 400 core\n"
	...
	char path3[] = PATH_TO_OBJECTS "/sphere_coarse.obj";
	simpleDepthShader.use();
	Object sphere3(path3);
	sphere3.makeObject(simpleDepthShader);

	Object rocket2(pathrocket);
	rocket2.makeObject(simpleDepthShader);	
	
	rocket2.model = glm::translate(rocket.model, glm::vec3(0.0, 0.0, 3.0));
	rocket2.model = glm::scale(rocket.model, glm::vec3(.5,.5,.5));
	...
	while (!glfwWindowShouldClose(window)) {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO);
			...
			// cubes
			model = glm::mat4(1.0f);
			model = glm::translate(model, glm::vec3(0.0f, 0.f, 0.0));
			...
			simpleDepthShader.setMatrix4("model", sphere2.model);
			sphere3.draw();
			...
			simpleDepthShader.setMatrix4("model", rocket2.model);		
			rocket2.draw();	
